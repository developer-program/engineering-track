...
After select image, you can select an Instance type. Each Instance type will have a different CPU, Memory, Storage configurations.
...
Add a tag comprise case sensitive key-value pair.
...
The IP address is available on `public IP` in the info section.
![ip address](_media/ec2-ip.png)

When you create an EC2 instance, the instance will have a private and public IP.

Private IP are IP address in a subnet, not directly connected to the internet, usually hide behind an ISP or a router. The allowable private IP range set by Internet Assigned Numbers Authority are:
...
SSH only works on Mac, Linux and Windows 10. If you don't use either of those, you will fail back to PUTTY.
...
At additional cost, you can rent an IP address and assign to an EC2 instance. When an instance stop and start, you will get the same IP on the EC2 instance. Most people won't use it.
...
1. Create an EC2 instance with Amazon Linux 2 image.
...
By default, Nginx give a startup page. Try hitting the IP address on the browser and realise that it loads nothing.

3. Update Security Group

We cannot access the Nginx webpage because we are blocking incoming traffic to our EC2 instance. To allow incoming traffic, we need to add inbound rules.

![security group](_media/security_group.png)

after adding the security rule, see the Nginx page.
...
Detailed instructions is available on [aws-developer-guide](https://docs.aws.amazon.com/sdk-for-javascript/v2/developer-guide/setting-up-node-on-ec2-instance.html)
